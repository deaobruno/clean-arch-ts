...
describe('/adapters/controllers/AuthenticatedController.ts', () => {
  ...
  it('should return successfully when authenticated', async () => {
    sinon.stub(validateAuthenticationUseCase, 'exec')
      .resolves({
        user: {
    ...
    const customerController = new CustomController({ useCase, validateAuthenticationUseCase })
    const result = await customerController.handle({ authorization: 'Bearer token' }, {})
  ...
  it('should throw error when ValidateAuthenticationUseCase is not passed to AuthenticatedController', async () => {
    ...
    expect(() => new CustomController({ useCase })).throw('[CustomController] Authentication use case is required')
  })
})
